...
updateChatText(chatbox) {
    ...
    this.messages.slice().reverse().forEach(function({item, index:number}) {
        if(item.name === "Sam")
        {
            html += '<div class="messages___item messages__item--visitor">' + item.message + '</div>'
        }
        else
        {
            html += '<div class="messages___item messages__item--operator">' + item.message + '</div>'
        }
    });
